...
export default function LoginPage() {
  ...
  const isShowTrendingSection =
    wyraList.filter((wyra: any) => wyra.likeCount > 10).length > 0;
    ...
    <div className="w-full flex flex-col items-center mt-20 mb-20">
      ...
      <div
        className={`grid grid-cols-3 ${
          isShowTrendingSection ? "lg:w-[30%]" : "lg:w-[35%]"
        } gap-4 mb-8 animate-slide-in-left`}
      >
      ...
      >
        ...
        <div
          className={`w-full md:w-1/2 ${
            isShowTrendingSection ? "md:pr-4" : ""
          } flex flex-col items-center`}
        >
          {/* Login Card */}
          <Card
            className={`shadow-2xl border-0 bg-white dark:bg-black/80 backdrop-blur-lg animate-slide-in-right w-full ${
              isShowTrendingSection ? "max-w-md" : ""
            }`}
          >
                        ...
                        <div className="flex justify-between">
                          <div className="mt-4 text-sm text-gray-500 dark:text-gray-200 flex gap-4">
                            <span className="flex items-center px-3 py-1 rounded-full text-sm font-medium transition bg-green-200 text-gray-800 dark:text-gray-200 hover:bg-green-300">
                              <ThumbsUp className="w-4 h-4 mr-1" size={18} />{" "}
                              {wyra?.likeCount}{" "}
                            </span>
                            <span className="flex items-center px-3 py-1 rounded-full text-sm font-medium transition bg-red-200 text-gray-800 dark:text-gray-200 hover:bg-red-300">
                              <ThumbsDown className="w-4 h-4 mr-1" size={18} />{" "}
                              {wyra?.dislikeCount}
                            </span>
