      ...
      {activeTab === "trending" ? (
        ...
        <Card className="shadow-2xl border-0 bg-white/80 backdrop-blur-lg animate-slide-in-right">
          ...
          <CardContent>
            {circles?.map((circle) => (
              <div key={circle.id}
                className="bg-white rounded-xl mb-3 border shadow-sm hover:shadow-md transition cursor-pointer flex  text-center gap-4 px-4 py-3">
                <div>
                  {circle.icon ? (
                    <img
                      src={circle.icon}
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-gray-500 flex items-center gap-1 w-[100px]">
                      <Users size={14} /> {circle.circle_members?.length || 0}{" "}
                      members
                    </p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-4">
                  {
                    circle.circle_members?.map((members: any, index: any) => {
                      return (
                        <div
                          key={index}
                          className="flex border border-[#ccc] p-2 rounded h-16 w-[180px]"
                        >
                          {members?.user_profile?.avatar ? (
                            <img
                              src={members?.user_profile?.avatar}
                              alt={`${members?.user_profile?.firstname} icon`}
                              className="w-10 h-10 rounded-full object-cover shadow-md border-4 border-white"
                            />
                          ) : (
                            <div className="w-10 h-10 rounded-full bg-gradient-to-r from-blue-500 to-purple-600 flex items-center justify-center shadow-md border-4 border-white">
                              <div className="w-full h-full bg-white rounded-full flex justify-center items-center">
                                <span className="text-2xl font-bold text-gray-600">
                                  {members?.user_profile?.firstname?.[0]?.toUpperCase()}
                                </span>
                              </div>
                            </div>
                          )}
                          <div className="ml-2 text-left">
                            <h3 className="text-sm font-semibold text-gray-900 mt-1">
                              {members?.user_profile?.firstname?.toUpperCase() + " " + members?.user_profile?.lastname?.toUpperCase()}
                            </h3>
                            <p className="text-[12px] text-gray-500 w-[100px]">
                              @{members?.user_profile?.username}
                            </p>
                          </div>
                        </div>
                      )
