...
import ReportWyraModal from "./ReportWyraModal";
...
export default function WyraSection({
  ...
  const [showEditWyraModal, setShowEditWyraModal] = useState({
    isShow: false,
    id: "",
  });
  ...
  const [blockModal, setBlockModal] = useState<any>({
    isOpen: false,
    userId: null,
    action: "",
  });
  ...
  const [expandedWyras, setExpandedWyras] = useState<{
    [key: string]: boolean;
  }>({});
  ...
  //         if (wyra?.created_by && wyra.created_by !== user.id) {

  //             const isAllowed = await isNotificationAllowed(wyra.created_by, "respond_my_wyra")
  //             if (isAllowed) {
  //                 await supabase.from("notifications").insert([
  //                     {
  //                         type: "respondWyra",
  ...
  //                         post_id: wyraId,
  //                         message: "has given a Why to your Wyra",
  //                     },
  //                 ]);
  //             }
  //         }
  ...
  const handleSubmitWyraOption = async (wyraId: any) => {
    ...
    if (error) {
      ...
      // Keep the why box open and show the submitted data
      setIsShowWhyReasonContainer((prev) => ({
        ...prev,
        [wyraId]: true,
      }));

      setWhyText(data?.[0]?.why || whyText);

      // Mark as submitted to disable input if needed
  ...
  return (
    <>
      <div className="w-full space-y-6">
        {wyras.map((wyra: any, index: any) => (
          <div key={`${wyra.id}-${index}`}>
            <Card
              className={`shadow-md hover:shadow-2xl border-0 bg-white dark:bg-black/80 backdrop-blur-lg transition-all pt-4 animate-slide-in-right ${
                postId === wyra.id ? "shadow-2xl shadow-blue-500" : ""
              }`}
            >
              <CardContent>
                <div className="flex md:gap-2">
                    ...
                    onClick={() => {
                      if (!disableActions) {
                        setActiveTab("user-profile"),
                          setSelectedUserId(wyra.creator?.id);
                      }
                    }}
                  ...
                  >
                    <div className="w-12 h-12 rounded-full bg-gray-200 dark:bg-gray-800  relative">
                      <CustomAvatar
                        userId={wyra.creator?.id}
                        firstName={wyra.creator.firstname}
                        lastName={wyra.creator.lastname}
                      />
                      ...
                      <h2 className="text-lg font-bold text-black dark:text-white">
                        ...
                        <span>
                          <span className="font-bold text-md mt-6">
                            {" "}
                            Asked
                            {wyra?.wyra_circles?.length > 0 &&
                              wyra?.wyra_circles[0]?.circle?.name &&
                              " in"}
                            {wyra?.wyra_circles?.length > 0 &&
                              wyra?.wyra_circles?.map(
                                (item: any, index: any) => {
                                  return (
                                    <span key={index}>
                                      {item?.circle?.name && (
                              ...
                              <DropdownMenuItem
                                ...
                                onClick={() =>
                                  setShowReportModal({
                                    isShow: true,
                                    id: wyra?.id,
                                    username: wyra?.creator.username,
                                  })
                                }
                              >
                ...
                <div className="flex flex-col md:flex-row items-center justify-center md:gap-4 gap-2">
                  {wyra.wyra_option
                    ...
                    .map((opt: any, index: number) => {
                      ...
                      const isSelected = !selectedOptions[wyra.id]
                        ? match?.selected_option_id === opt.id
          ...
          `}
                            onClick={() => {
                              ...
                              if (match?.selected_option_id === opt.id) {
                                ...
                                setIsShowWhyReasonContainer((prev) => ({
                                  ...prev,
                                  [wyra.id]:
                                    prev[wyra.id] && isSelected ? false : true, // toggle open/close per wyra
                                }));
                      ...
                      <div className="border mb-2 p-2 rounded-lg">
                        {(expandedWyras[wyra.id]
                          ...
                          return (
                            <div
                              className={`my-2 ${
                                item?.user_profiles?.account_settings
                                  ?.multi_color_why_boxes
                                  ? "border rounded-lg bg-gradient-to-br from-blue-500 to-purple-600 text-white dark:text-black shadow-lg p-2"
                                  : "border-b"
                              }`}
                              key={item?.id}
                            >
                              <div
                                onClick={() => {
                                  if (disableActions) return;
                                  setActiveTab("user-profile"),
                                    setSelectedUserId(item?.user_profiles?.id);
                                }}
                                className="flex cursor-pointer items-center w-max"
                              >
                                <div className="relative w-12 h-12 rounded-full mr-2">
                                  <CustomAvatar
                                    userId={item?.user_profiles?.id}
                                    firstName={item?.user_profiles.firstname}
                                    lastName={item?.user_profiles.lastname}
                                  />
                                </div>
                              ...
                              <div className="ml-12 rounded">
                                <div className="ml-2">
                                  <div className="flex items-center my-2">
                                    ...
                                    <h3 className="mx-4 text-lg font-bold whitespace-nowrap">
                                      Why?
                                    </h3>
                                ...
                                <div className="my-2 flex justify-center gap-2">
                                  <WyraSelectedOptionLikeButton
                                    ...
                                    userId={item?.user_profiles?.id}
                                    count={
                                      item.wyra_selected_option_reaction?.filter(
                                        (r: any) => r.type === "like"
                                      ).length || 0
                                    }
                                  />
                ...
                <div className="flex justify-between">
                  <div className="flex items-center gap-2 mt-5">
                    <LikeButton
                      wyraId={wyra.id}
                      isFloatAllow={wyra.settings?.animate_floating_effects}
                      userId={user?.id}
                      count={wyra?.likeCount}
        ...
        <ModalContent>
          ...
          <ModalFooter>
            ...
            <Button
              className={
                blockModal?.action === "block"
                  ? "bg-red-600 hover:bg-red-700 text-white dark:text-black"
                  : ""
              }
              onClick={async () => {
      ...
      <ReportWyraModal
        showReportModal={showReportModal.isShow}
        setShowReportModal={setShowReportModal}
        username={showReportModal?.username}
        wyraId={showReportModal.id}
        reporterData={user}
      />
    </>
  );
